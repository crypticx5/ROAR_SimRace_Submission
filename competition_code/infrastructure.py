...
class ManualControlViewer:
    """
    A simple Pygame-based viewer that allows manual control of the car for
    debugging.  It displays the RGB image (and optionally the occupancy map)
    from the camera sensor and returns control commands based on keyboard
    input.  This is copied from the official competition skeleton and must
    remain unmodified.
    """
    ...
    def init_pygame(self, x: int, y: int) -> None:
        pygame.init()
...
class RoarCompetitionAgentWrapper(RoarPyActor):
    """
    A thin wrapper around the underlying RoarPyActor.  It is required by
    the competition evaluation code but simply forwards calls to the
    underlying actor.  This is copied from the official competition
    skeleton and must remain unmodified.
    """

    def __init__(self, wrapped: RoarPyActor) -> None:
